...
started/CLI examples describe) — hover a token to spotlight what it relates to, click it for
details:
...
(solid), so you can tell at a glance which kind of reference it is — click `border.focusring`
below and the detail panel names the `color` member the reference comes from:
