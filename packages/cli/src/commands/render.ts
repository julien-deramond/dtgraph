import { writeFile } from 'node:fs/promises';

import { renderTokenGraphToSvg } from '@dtgraph/core';
import type { Command } from 'commander';

import { loadAndResolveTokenFiles, readTokenFiles, type TokenFileInput } from '../token-graph.js';

export interface RenderCommandOptions {
  output?: string;
}

/** Parse, resolve, and render one or more DTCG token file contents to an SVG string. */
export function renderTokenFilesToSvg(files: TokenFileInput[]): string {
  return renderTokenGraphToSvg(loadAndResolveTokenFiles(files));
}

export function registerRenderCommand(program: Command): void {
  program
    .command('render')
    .description('Render one or more DTCG token files to an SVG token graph')
    .argument('<files...>', 'DTCG token JSON file(s) to render')
    .option('-o, --output <file>', 'write the SVG to this file instead of stdout')
    .action(async (files: string[], options: RenderCommandOptions) => {
      const svg = renderTokenFilesToSvg(await readTokenFiles(files));
      if (options.output !== undefined) {
        await writeFile(options.output, svg, 'utf8');
      } else {
        process.stdout.write(`${svg}\n`);
      }
    });
}
