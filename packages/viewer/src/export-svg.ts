...
function renderNode(node: PlacedNode): string {
  return (
    ...
    `fill="${node.color}"><title>${escapeXml(node.key)}</title></circle>`
  );
}
 ...
 *
 * This is the picture, not the session: the camera, the hover spotlight, the selection and the
 * chrome (search, legend, detail panel) are all left out, so the file is what the map looks like
 * at rest. Pass the graph from a mounted viewer (`viewer.graph`), or build and lay one out
 * yourself with `buildViewerGraph` + `layoutViewerGraph`.
 *
...
export function renderViewerGraphToSvg(graph: ViewerGraph, options: ExportSvgOptions = {}): string {
  ...
  graph.forEachEdge((_edge, attrs, source, target) => {
    ...
    edgeMarkup.push(
      renderEdge(
        from,
        to,
        fadeTowards(attrs.color, theme.background, theme.edgeStrength),
        Math.max(0.6, attrs.size),
      ),
    );
  });

  const painted = [...nodes].sort((a, b) => a.zIndex - b.zIndex || a.size - b.size);
  const labelled =
    labels === 'none'
      ? new Set<string>()
      : labels === 'all'
        ? new Set(nodes.map((node) => node.key))
        : selectGridLabels(nodes, LABEL_GRID_CELL_SIZE);
  ...
  const title =
    ...
    `${graph.size} reference${graph.size === 1 ? '' : 's'}</title>`;
  ...
  return (
    ...
    title +
    background +
    `<g class="dtgraph-edges">${edgeMarkup.join('')}</g>` +
    `<g class="dtgraph-nodes">${painted.map(renderNode).join('')}</g>` +
    `<g class="dtgraph-labels">` +
    painted
      .filter((node) => labelled.has(node.key) && node.label !== '')
